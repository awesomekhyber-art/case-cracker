...
DEFINITION QUALITY — IMPORTANT BUDGET RULE: apply the richer 3-part structure (plain-language meaning + a quick vivid mental image "think ___" + a phonetic/spelling hook tying part of the term's own sound to its meaning) to AT MOST 8 terms total across the entire case — pick the 8 most genuinely confusing or highest-yield ones. This cap exists because generating many long definitions makes the response too slow — respect it strictly, do not exceed 8. EVERY OTHER marked term (there will usually be several more beyond those 8) still gets glossary-marked as before, but with a SHORT plain-language definition only, one clause, under 15 words, no mental image, no phonetic hook. Example of the rich (capped-at-8) style: ⟦mydriasis|abnormal dilation of the pupil. Think big eyes. Think myDRIasis for Dilation⟧. Example of the plain style used for all the rest: ⟦chemotaxis|movement of cells toward a chemical signal⟧. Keep the plain ones genuinely short — the word budget saved there is what makes room for the 8 rich ones without the response getting too slow to generate in time.
